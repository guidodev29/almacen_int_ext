package edu.sv.ufg.fis.amb.almacenamiento_int_ext
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        escrituraArchivosAlmacenamientoInterno(this,"archivo_almacenamiento_interno.txt", "ESTE ES UN CONTENIDO EN ALMACENAMIENTO INTERNO")
        escrituraArchivosAlmacenamientoExterno(this,"archivo_almacenamiento_externo.txt", "ESTE ES UN CONTENIDO EN ALMACENAMIENTO EXTERNO")

        boton = findViewById<Button>(R.id.btn_guardar)
        texto = findViewById<EditText>(R.id.txt_data)

        boton.setOnClickListener{
            escrituraArchivosAlmacenamientoInterno(this, "archivos_con_valor_campo_texto.txt", texto.text.toString())
    ...
    fun escrituraArchivosAlmacenamientoExterno(context: Context, fileName: String, content: String ){
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q){
            ...
            try {
                file.writeText(content)
                Log.v("ESCRITURA EN ALMCENAMIENTO EXTERNO","RUTA: `${filePath}`")
            }catch (e: Exception){
                e.printStackTrace()
            }
        }else{
